...
object Queries {
    fun queryNotes(openDatabaseHelper: SQLiteOpenHelper, title: String): ArrayList<Note> {
        ...
        val projections =
            arrayOf(BaseColumns._ID, Contract.NoteEntry.COLUMN_NAME_TITLE, Contract.NoteEntry.COLUMN_NAME_DESCRIPTION)
        ...
        if (cursor.moveToFirst()) {
            do {
                val noteId = cursor.getInt(cursor.getColumnIndex(BaseColumns._ID))
